...
$stmt = $pdo->prepare('SELECT t.*, COUNT(ti.id) AS total_products FROM transactions t JOIN transactions_items ti ON ti.txn_id = t.txn_id WHERE cast(t.created as DATE) = cast("' . $date . '" as DATE) GROUP BY t.id, t.txn_id, t.payment_amount, t.payment_status, t.created, t.payer_email, t.first_name, t.last_name, t.address_street, t.address_city, t.address_state, t.address_zip, t.address_country, t.account_id, t.payment_method, t.discount_code, t.shipping_method, t.shipping_amount ORDER BY t.created DESC');
$stmt->execute();
$orders = $stmt->fetchAll(PDO::FETCH_ASSOC);
// Get the orders statistics
$stmt = $pdo->prepare('SELECT SUM(payment_amount) AS earnings FROM transactions WHERE (payment_status = "Completed" OR payment_status = "Subscribed") AND cast(created as DATE) = cast("' . $date . '" as DATE)');
$stmt->execute();
...
<div class="dashboard">
    <div class="content-block stat">
        <div class="data">
            ...
            <p><?=$orders ? number_format(count($orders)) : 0?></p>
        </div>
        <div class="icon">
    ...
    <div class="content-block stat green">
        <div class="data">
            ...
            <p><?=currency_code?><?=$order_stats['earnings'] ? number_format($order_stats['earnings'], 2) : 0.00?></p>
        </div>
        <div class="icon">
    ...
    <div class="content-block stat cyan">
        <div class="data">
            ...
            <p><?=$accounts['total'] ? number_format($accounts['total']) : 0?></p>
        </div>
        <div class="icon">
    ...
    <div class="content-block stat red">
        <div class="data">
            ...
            <p><?=$products['total'] ? number_format($products['total']) : 0?></p>
        </div>
        <div class="icon">
