    ...
    if (isset($_POST['item_id']) && is_array($_POST['item_id']) && count($_POST['item_id']) > 0) {
        ...
        for ($i = 0; $i < count($_POST['item_id']); $i++) {
            ...
            if (!intval($_POST['item_id'][$i])) {
                ...
                $stmt = $pdo->prepare('UPDATE transaction_items SET txn_id = ?, item_id = ?, item_price = ?, item_quantity = ?, item_options = ? WHERE id = ?');
        ...
        $stmt = $pdo->prepare('DELETE FROM transaction_items WHERE txn_id = ? AND id NOT IN (' . $in . ')');
        $stmt->execute(array_merge([ $txn_id ], $delete_list));
    } else {
        // No item exists, delete all
        $stmt = $pdo->prepare('DELETE FROM transaction_items WHERE txn_id = ?');
        $stmt->execute([ $txn_id ]);       
    }
}
// Save captured data
if (isset($_GET['id'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM transaction_items WHERE txn_id = ?');
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $pdo->prepare('UPDATE transactions SET txn_id = ?, payment_amount = ?, payment_status = ?, created = ?, payer_email = ?, first_name = ?, last_name = ?, address_street = ?, address_city = ?, address_state = ?, address_zip = ?, address_country = ?, account_id = ?, payment_method = ?, discount_code = ?, shipping_method = ?, shipping_amount = ?, tax_amount = ? WHERE id = ?');
        $stmt->execute([ $_POST['txn_id'], $_POST['amount'], $_POST['status'], date('Y-m-d H:i:s', strtotime($_POST['created'])), $_POST['email'], $_POST['first_name'], $_POST['last_name'], $_POST['address_street'], $_POST['address_city'], $_POST['address_state'], $_POST['address_zip'], $_POST['address_country'], empty($_POST['account']) ? NULL : $_POST['account'], $_POST['method'], $_POST['discount_code'], $_POST['shipping_method'], $_POST['shipping_amount'] ? $_POST['shipping_amount'] : 0, $_POST['tax_amount'] ? $_POST['tax_amount'] : 0, $_GET['id'] ]);
    ...
    $page = 'Create';
    if (isset($_POST['submit'])) {
        $stmt = $pdo->prepare('INSERT INTO transactions (txn_id,payment_amount,payment_status,created,payer_email,first_name,last_name,address_street,address_city,address_state,address_zip,address_country,account_id,payment_method,discount_code,shipping_method,shipping_amount,tax_amount) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
        $stmt->execute([ $_POST['txn_id'], $_POST['amount'], $_POST['status'], date('Y-m-d H:i:s', strtotime($_POST['created'])), $_POST['email'], $_POST['first_name'], $_POST['last_name'], $_POST['address_street'], $_POST['address_city'], $_POST['address_state'], $_POST['address_zip'], $_POST['address_country'], empty($_POST['account']) ? NULL : $_POST['account'], $_POST['method'], $_POST['discount_code'], $_POST['shipping_method'], $_POST['shipping_amount'] ? $_POST['shipping_amount'] : 0, $_POST['tax_amount'] ? $_POST['tax_amount'] : 0, ]);
        addOrderItems($pdo, $_POST['txn_id']);
...
<form method="post">
    ...
    <div class="content-title">
        ...
        <div class="btns">
            ...
            <?php if ($page == 'Edit'): ?>
            <input type="submit" name="delete" value="Delete" class="btn red mar-right-1" onclick="return confirm('Are you sure you want to delete this order?')">
            <?php endif; ?>
    ...
    <div class="content-block tab-content active">
        ...
        <div class="form responsive-width-100">
            ...
            <div class="group">
                <div class="item">
                    <label for="method"><span class="required">*</span> Payment Method</label>
                    <input id="method" type="text" name="method" placeholder="website" value="<?=$transaction['payment_method']?>" required>
                </div>
                <div class="item">
            ...
            <div class="group">
                ...
                <div class="item">
                    ...
                    <input id="shipping_amount" type="number" name="shipping_amount" placeholder="0.00" value="<?=$transaction['shipping_amount']?>" step=".01">
                </div>
            </div>

            <label for="tax_amount">Tax Amount (fixed)</label>
            <input id="tax_amount" type="number" name="tax_amount" placeholder="0.00" value="<?=$transaction['tax_amount']?>" step=".01">
    ...
    <div class="content-block tab-content no-pad">
                    ...
                    <tr>
                        <td>
                            ...
                            <select name="item_product[]" class="item-product">
                                ...
                                <option data-price="<?=$product['price']?>" value="<?=$product['id']?>"<?=$item['item_id']==$product['id']?' selected':''?>><?=$product['id']?> - <?=htmlspecialchars($product['title'], ENT_QUOTES)?></option>
                                <?php endforeach; ?>
                            </select>
                        </td>
                        <td><input name="item_price[]" type="number" placeholder="Price" value="<?=$item['item_price']?>" step=".01" class="item-price"></td>
                        <td><input name="item_quantity[]" type="number" placeholder="Quantity" value="<?=$item['item_quantity']?>" class="item-quantity"></td>
                        <td><input name="item_options[]" type="text" placeholder="Options" value="<?=htmlspecialchars($item['item_options'], ENT_QUOTES)?>"></td>
                        <td><svg class="delete-item" width="14" height="14" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><title>Delete</title><path d="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z" /></svg></td>
                    </tr>
                    <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
...
<?=template_admin_footer('<script>initManageOrder(' . json_encode($products) . ')</script>')?>