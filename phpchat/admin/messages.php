...
$stmt->execute([ $_SESSION['chat_account_id'], $_SESSION['chat_account_id'] ]);
...
<div class="content-title">
    <div class="title">
        ...
        <div class="txt">
            <h2>Conversations</h2>
            <p>View, manage, and search converstaions.</p>
        </div>
    </div>
</div>
    ...
    <div class="conversations">
        <div class="list">
            ...
            <div class="users scroll">
                ...
                <?php 
                $which = $c['account_sender_id'] != $_SESSION['chat_account_id'] ? 'sender' : 'receiver'; 
                ?>
                <a href="#" class="user<?=isset($_GET['id']) && $_GET['id'] == $c['id']?' selected':''?>" data-id="<?=$c['id']?>" data-accountid="<?=$c['account_' . $which . '_id']?>" data-lastseen="Last seen <?=date('d/m/Y', strtotime($c['account_' . $which . '_last_seen']))?> at <?=date('H:i', strtotime($c['account_' . $which . '_last_seen']))?>" title="<?=date('Y-m-d H:i:s') > date('Y-m-d H:i:s', strtotime($c['account_' . $which . '_last_seen'] . ' + 5 minute'))?'Offline':$c['account_' . $which . '_status']?>">
                    ...
                    <div class="details">
                        ...
                        <p class="<?=$c['account_' . $which . '_unread']>0?'unread':''?>"><?=$c['msg'] ? decode_emojis(htmlspecialchars(str_replace(['{name}', '{email}'], [$c['account_' . $which . '_full_name'], $c['account_' . $which . '_email']], $c['msg']), ENT_QUOTES)) : ''?></p>
                    </div>
                    <?php if ($c['msg_date']): ?>
